...
``Session`` / ``AgentClient`` / ``Hub`` into ``context.dependencies``
...
``TaskInject`` lives in framework-core (``autogen.beta.annotations``)
because ``Task`` itself is framework-core; it is re-exported here for
symmetry with the network-only injects so users can ``from
autogen.beta.network.client.inject import TaskInject`` alongside the
others.
"""
...
# ``autogen.beta.task.TaskInject`` — both resolve to ``ag2.task``.
